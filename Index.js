...
console.log(str1.replace(/([-_])(.)/g,(a, b, c, d, e) => {
	...
	// Элементом считается то, что в круглых скобках, по этому, если первый элемент не нужен, то
	// можно взять в скобки второй, а первый не брать и тогда первым будет то, что в скобках
	// Например [-_](.) - тут первый элемент будет (.)
